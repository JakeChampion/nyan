...
pub fn get(store_name: &str, request: Request) -> Result<Option<Response>, Error> {
    ...
    match item {
        ...
        Some(item) => {
            ...
            headers.insert("etag", metadata.etag.parse()?);
            headers.insert("last-modified", metadata.last_modified.parse()?);
                    ...
                    match HttpRange::parse(range, total.try_into()?) {
                        Ok(subranges) => {
                            if subranges.len() == 1 {
                                ...
                                body.write_bytes(boundary);
                                length += boundary.len();
                                headers.insert("content-length", length.to_string().parse()?);
                                let mut response = Response::from_status(206);
                                for (name, value) in headers {
                                    response.set_header(name.expect("name is a HeaderName"), value);
                                }
    ...
    if let Some(header) = header {
        if !if_none_match(metadata, header.to_str()?) {
            if method == get || method == head {
                ...
                response.set_header("etag", metadata.etag.parse::<http::HeaderValue>()?);
                response.set_header(
                    "last-modified",
                    metadata.last_modified.parse::<http::HeaderValue>()?,
                );

                response.set_header("accept-ranges", "bytes".parse::<http::HeaderValue>()?);

                if let Some(content_type) = &metadata.content_type {
                    response.set_header("content-type", content_type.parse::<http::HeaderValue>()?);
                }
        ...
        if method == get || method == head {
            ...
            if let Some(header) = header {
                if !if_modified_since(metadata, header.to_str()?) {
                    ...
                    if let Some(content_type) = &metadata.content_type {
                        response
                            .set_header("content-type", content_type.parse::<http::HeaderValue>()?);
                    }
                    return Ok((Some(response), request));
                }
            }
...
fn if_match(validation_fields: &Metadata, header: &str) -> bool {
    ...
    if header == "*" {
        ...
        if split_list(header)
            .into_iter()
            .any(|etag| strong_match(etag, &validation_fields.etag))
        {
            return true;
        }
    }
...
fn if_modified_since(validation_fields: &Metadata, header: &str) -> bool {
    ...
    if httpdate::parse_http_date(&validation_fields.last_modified)
        .expect("validation_fields.last_modified is valid HTTP-date")
        <= date.expect("date is valid HTTP-date")
    {
        return false;
    }
